# app.py
# ------------------------------------------------------------------
# BABS Intake-assistent - screenshot (oproep) -> auto-intake + tekst
# + chatdossier (ASCII-safe, getest met Python 3.11 + Streamlit 1.50.0)
# ------------------------------------------------------------------
...
# Optional OCR (pytesseract). In the cloud the Tesseract binary is usually not available.
# The app works fine without OCR; this flag will disable the OCR UI automatically.
try:
...
st.set_page_config(page_title="BABS Intake-assistent", page_icon=None, layout="centered")

# Visual ping that the app actually loaded
st.success("App geladen - server draait")
st.caption(f"Python: {sys.version.split()[0]} | Streamlit: {st.__version__}")

# ------------------------- State -------------------------
if "threads" not in st.session_state:
    st.session_state["threads"] = {}  # key: slug, value: dict(meta, chat)
...
def parse_phone(text: str) -> str | None:
    # 06-12345678, +31 6 1234 5678, 06 12 34 56 78
...
def parse_names(text: str) -> tuple[str | None, str | None]:
    # After a closing like "Groeten," capture "Anna en Bram"
    m = re.search(
        r"(?:groeten|groet|hartelijke groet|liefs)[\s,:-]*\n?\s*([A-Za-zÀ-ÿ'\-]+)\s*(?:&|en)\s*([A-Za-zÀ-ÿ'\-]+)",
        text, flags=re.I
    )
    if m:
        return m.group(1), m.group(2)
    # "Wij zijn Anna en Bram"
    m = re.search(r"wij\s+zijn\s+([A-Za-zÀ-ÿ'\-]+)\s*(?:&|en)\s*([A-Za-zÀ-ÿ'\-]+)", text, flags=re.I)
    if m:
        return m.group(1), m.group(2)
    # Fallback: two capitalized lines
...
STYLE_SIG = (
    "Arwen van Gestel\n"
    "Trouwambtenaar / BABS\n"
    "E-mail: arwenvangestel@gmail.com\n"
)

VOORWAARDEN = (
    "Voorwaarden (samenvatting):\n"
    "- Vergoeding: EUR 690\n"
    "- Reiskosten: EUR 0,15 per km vanaf Roosendaal (per factuur)\n"
    "- Betaling: uiterlijk 10 dagen voor de ceremonie\n"
...
with st.expander("1) Upload screenshot van de oproep (optioneel)"):
    ...
    if up is not None:
        ...
        if OCR_AVAILABLE and st.checkbox("Probeer tekst extraheren (OCR)"):
            try:
                ...
                st.warning(f"OCR niet beschikbaar: {e}")
...
def bericht_eerste_contact(n1, n2, datum, locatie, toon, extra, telmail):
    ...
    parts = []
    if datum:
        parts.append(f"voor {datum}")
    if locatie:
        parts.append(f"in {locatie}")
    dl = " ".join(parts).strip()
    ...
    cta = "Zullen we kort kennis maken? Stuur een paar momenten die passen; een eerste kennismaking plan ik graag met jullie in."
    body = f"""{aanhef}
...
def bericht_bevestiging(n1, n2, datum, locatie, extra):
    ...
    whenwhere = []
    if datum:
        whenwhere.append(f"Datum: {datum}")
    if locatie:
        whenwhere.append(f"Locatie: {locatie}")
    ww = "; ".join(whenwhere)
    zin = "Leuk dat we met elkaar aan de slag gaan!" + (f" ({ww})" if ww else "")
...
colb = st.columns(3)
if colb[0].button("Eerste kennismail"):
...
if "last_text" in st.session_state:
    ...
    st.download_button("Download als .txt", data=st.session_state["preview_text"].encode("utf-8"), file_name=f"{slug}.txt")
...
if slug not in st.session_state["threads"]:
    st.session_state["threads"][slug] = {
        ...
        "meta": {
            "partner1": n1, "partner2": n2, "datum": datum, "locatie": locatie,
            "email": mail, "tel": tel, "slug": slug, "created": datetime.now().isoformat(timespec="seconds")
        },
        "chat": [],
    }
...
if submitted and msg.strip():
    st.session_state["threads"][slug]["chat"].append({
...
if chat:
    for item in reversed(chat):
        with st.chat_message("user" if item["from"] != "Arwen" else "assistant"):
            st.markdown(f"**{item['from']}** · _{item['ts']}_\n\n{item['text']}")
else:
...
if colx[0].button("Voeg huidige tekst toe aan chat") and "preview_text" in st.session_state:
    st.session_state["threads"][slug]["chat"].append({
        "ts": datetime.now().isoformat(timespec="seconds"),
        "from": "Arwen",
        "text": st.session_state["preview_text"]
    })
if colx[1].button("Exporteer dossier (JSON)"):
    b = BytesIO()
